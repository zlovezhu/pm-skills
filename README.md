...
| **prd-writer** | 按两级编号 + 列表项体系起草 PRD，支持版本管理 + 克制原则 + 可选 HTML 低保真原型 | "写需求"、"PRD"、"产品需求文档" |
...
│   ├── SKILL.md
│   └── references/
│       ├── prd_example.md             # 完整 PRD 范例（两级编号 + 列表项体系）
...
**Skill 行为**：
- 按两级编号 + 列表项体系起草（`### 模块` / `#### 功能点` / 再细的内容用 `-` 嵌套列表）
- 每个 1/2 级标题挂稳定锚点 `feat-X-Y`，方便变更追踪与跨章节引用
- 头部强制带版本表（版本 / 更新时间精确到分钟 / 改动摘要 / 影响范围 / 修改人）
- 改 PRD 时强制走「读旧版 → 追加变更行 → 用户确认 → 改正文」，不允许跳步
- 内置克制原则：默认精简模式不渲染背景/目标/范围、不重复解释已说过的逻辑、信息不足时留 ❓ 占位追问而不是编造
